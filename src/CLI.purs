...
windows = many $ option windowReader (long "window" <> short 'w' <> metavar "MEMBERSHIP_RESOURCE MEMBER_RELATION CONTENT_TIMESTAMP_RELATION START END" <> help "A window for the Stream Container to start with.")
...
windowReader = eitherReader parse
  where
    ...
    parse string = case split (Pattern " ") string of 
      [ membershipResource, memberRelation, contentTimestampRelation, startString, endString ] -> case fromString startString of
        ...
        Just start -> case fromString endString of 
          ...
          Just end -> Right $ Window (namedNode membershipResource) (namedNode memberRelation) (namedNode contentTimestampRelation) (Milliseconds $ toNumber start) (Milliseconds $ toNumber end)
      strArr -> Left $ "Window specifications needs 5 compontens, " <> show (length strArr) <> " were given!"
